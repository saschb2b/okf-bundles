---
type: Rechts-Plugin
title: "Regulatorisches Recht"
description: "Aufsichtsrecht - KWG, ZAG, WpHG, GwG, EnWG, TKG, HeilMWerbG, Umsatzsteuer-Voranmeldung, Inkasso/RDG, Regulator-Feeds, Wochendigest."
resource: https://github.com/Klotzkette/claude-fuer-deutsches-recht/tree/main/regulatorisches-recht
tags: [regulatorisches, aufsichtsrecht, kwg, zag, wphg, gwg, enwg, tkg, heilmwerbg, umsatzsteuer-voranmeldung]
plugin_slug: regulatorisches-recht
rechtsgebiet: "Steuer-, Finanz- & Aufsichtsrecht"
skills: 61
version: "335.0.0"
timestamp: 2026-06-18T12:00:00Z
---

# Überblick

Aufsichtsrecht - KWG, ZAG, WpHG, GwG, EnWG, TKG, HeilMWerbG, Umsatzsteuer-Voranmeldung, Inkasso/RDG, Regulator-Feeds, Wochendigest.

Teil des Rechtsgebiets [Steuer-, Finanz- & Aufsichtsrecht](/steuer-finanz-aufsichtsrecht/index.md) in der Sammlung [claude-fuer-deutsches-recht](/references/claude-fuer-deutsches-recht.md). Das Plugin bündelt **61 Skills** (Stand v335.0.0).

# Abdeckung

`regulatorisches`, `aufsichtsrecht`, `kwg`, `zag`, `wphg`, `gwg`, `enwg`, `tkg`, `heilmwerbg`, `umsatzsteuer-voranmeldung`, `inkasso`, `rdg`.

# Skills (Auswahl)

Eine Auswahl der 61 enthaltenen Skills (vollständige Liste im [Quell-Repository](https://github.com/Klotzkette/claude-fuer-deutsches-recht/tree/main/regulatorisches-recht/skills)):

- `anhoerung-red-team-und-qualitaetskontrolle`
- `anschluss-router`
- `aufsichts-feed-monitor`
- `aufsichtskommunikation-grundregeln`
- `aufsichtsrecht-erstpruefung-und-mandatsziel`
- `aufsichtssanktion-revision-spezial`
- `aufsichtsverfahren-anhoerung-gwg`
- `aufsichtsverfahren-formular-portal-und-einreichung`
- `dokumente-intake`
- `dora-ikt-vertragspruefung`

# Citations

[1] [plugin.json](https://github.com/Klotzkette/claude-fuer-deutsches-recht/blob/main/regulatorisches-recht/.claude-plugin/plugin.json)
[2] [Marketplace-Manifest](https://github.com/Klotzkette/claude-fuer-deutsches-recht/blob/main/.claude-plugin/marketplace.json)

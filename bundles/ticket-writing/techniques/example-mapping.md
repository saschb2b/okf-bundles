---
type: Technique
title: Example Mapping
description: Matt Wynne's 25-minute, four-card refinement technique that surfaces a story's rules, concrete examples, and open questions.
resource: https://cucumber.io/blog/bdd/example-mapping-introduction/
tags: [refinement, bdd, acceptance-criteria]
generated:
  by: claude-code/unrecorded
  at: 2026-06-18T11:30:00Z
sources:
  - resource: https://cucumber.io/blog/bdd/example-mapping-introduction/
    title: "Introducing Example Mapping (Matt Wynne, Cucumber)"
---

# The four cards

- **Yellow**: the story.
- **Blue**: a rule or acceptance criterion.
- **Green**: a concrete example of a rule.
- **Red**: an open question nobody can answer yet.

# The built-in smell test

A small group should map a well-understood, well-sized story in about 25 minutes. If they cannot, it is too big or too uncertain. Many red cards mean too much is implicit (see [the Tapper](/smells/tapper.md)); many blue rules mean the story is too big (see [the Boulder](/smells/boulder.md)).

# Related

A concrete way to run [Three Amigos](/techniques/three-amigos.md). See the [reference](/references/example-mapping-wynne.md).

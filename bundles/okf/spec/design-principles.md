...
: OKF requires exactly one thing, a `type` per concept. Which types exist, what other fields appear, and how the body is organized are the producer's call. The spec defines the interoperability surface, not the content model. See [conformance](conformance.md).
...
- [Non-goals](non-goals.md) states the same boundaries as explicit exclusions.
- [Extensions](extensions.md) is minimal opinion applied to the key space.

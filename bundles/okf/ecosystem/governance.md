...
`A community index`
: `#65` and `#166` both ask for a Community and ecosystem tools section. Neither has landed, so [the tool list](/ecosystem/community-tools.md) exists only in a comment thread. Newcomers keep asking what they can use to author or validate a bundle, which is the question an index answers.
...
- **Stable but contested at the edges.** The [v0.2 trust families](/trust/why-trust-is-a-field.md). They will most likely gain fields rather than change meaning, and see [the four axes](/ecosystem/open-questions.md).
...
- **Pin the version you targeted.** Declare `okf_version` in the root index and record in [`log.md`](/spec/log-file.md) which revision you read. When §7 and §5.1 [contradict each other](/ecosystem/open-questions.md), the useful record is which one you followed.
...
- **Publish the tool and say so.** Several entries in [community tools](/ecosystem/community-tools.md) reached people only by opening an issue announcing themselves, since no index exists.

# Related

- [State of adoption](/ecosystem/adoption.md) is the measured picture this governance sits on top of.

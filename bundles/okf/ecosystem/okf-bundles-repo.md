...
# The two shapes it demonstrates
...
`A bulk corpus`
: One concept per record, reached through an index tree by id, date or norm. Sixty thousand court decisions cannot be hand-linked, so the leaves are legitimately index-navigable and the no-orphans expectation is relaxed. See [the corpus exception](/practice/graph-hygiene.md).

Knowing which shape a bundle is decides how it is validated and how an agent should read it.
...
- [`sources`](/trust/sources.md) has to carry a licensing story. Official texts can be reproduced, and commentary literature cannot. See [external references](/practice/external-references.md).
- A recipe bundle linking each ingredient back to the dishes it appears in answers "what can I cook with what I have?" by intersecting backlinks, which is [traversal](/spec/cross-linking.md) doing work no similarity search does.

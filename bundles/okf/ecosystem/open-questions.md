...
Prompt injection is worth flagging separately, because a bundle is text an agent reads and the spec currently says nothing about treating it as data rather than instructions. Later threads cite a "§12 trust-and-safety" section as though it were part of the spec. It is not. It exists in a community pull request open since 15 June, which is a good illustration of why [governance throughput](governance.md) matters for what people believe the format says.
...
The rule the thread settled on is worth keeping whatever the spec does: a profile warning is never core nonconformance, and a profile pass never rescues a core failure. Two verdicts, one bundle, no arithmetic between them. This is the mechanism [ODSF](llms-txt-and-sitemap.md) is an instance of, so a bundle profile for a domain is a pattern with tracker support rather than an invention.
...
The fix has been written three times. A community pull request closed the oldest of these issues on 15 June with the before-and-after diff, and two further contributors have since submitted the same fix independently. None has merged. See [governance](governance.md).
...
- [State of adoption](adoption.md) covers who is in these threads.
- [Governance](governance.md) covers who decides which of these lands.

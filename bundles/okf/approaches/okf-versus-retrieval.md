...
A bundle also improves the retrieval side rather than competing with it. Indexing whole concepts instead of arbitrary chunks removes [failure 01](retrieval-failure-modes.md), and the frontmatter gives the retriever fields to filter on before ranking. The four compose patterns and a diagnostic for whether your retrieval is actually failing are in [running retrieval and OKF together](retrieval-and-okf-together.md).
...
- [Four ways retrieval lets an agent down](retrieval-failure-modes.md) is the argument in full.

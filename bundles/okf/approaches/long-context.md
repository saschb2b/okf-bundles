...
Windows went from a few thousand tokens to millions. So stop curating, put the whole corpus in, and let the model sort it out. [Prompt stuffing](prompt-stuffing.md), scaled until the problem disappears.
...
: Put the current definition and the deprecated one in the same window and the model sees two plausible statements with no way to rank them. That is [retrieval failure 02](retrieval-failure-modes.md), unchanged. Nothing about size tells the model which one Finance approved, and only [`status` and `stale_after`](../trust/lifecycle.md) do.
...
Bigger windows are genuinely useful, and they move the boundary rather than removing it. They make [prompt stuffing](prompt-stuffing.md) viable for larger documents. They let an agent hold a longer traversal without re-reading. They reduce how aggressively [assembly](../context/context-assembly.md) has to prune.

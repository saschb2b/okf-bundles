...
: Point the existing pipeline at the bundle and set the chunk boundary to the file. A [concept](../spec/concept-document.md) is already a coherent unit, so a rule arrives with its caveat and a schema table arrives whole. This removes [failure 01](retrieval-failure-modes.md) with a configuration change rather than a rewrite.
...
A failure on the last two is the strong signal. They are the ones no amount of tuning, reranking or [window size](long-context.md) fixes, because the information was never in the corpus.

# What to move first

Not the whole corpus. Take the questions that failed the diagnostic, write the ten or twenty concepts that answer them, index those alongside everything you already have, and leave the long tail exactly where it is. The split is in [OKF versus retrieval](okf-versus-retrieval.md), and the starting procedure in [the adoption path](../practice/adoption-path.md).

# Related

- [Four ways retrieval lets an agent down](retrieval-failure-modes.md) is what the diagnostic tests for.
- [Retrieval-augmented generation](retrieval-augmented-generation.md) is the pipeline being extended.

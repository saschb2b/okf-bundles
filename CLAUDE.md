...
- Link with relative targets (`invest.md` for a sibling, `../techniques/invest.md` across directories), and put the relationship in the prose around the link, not in the link itself. The spec recommends the bundle-absolute form (`/techniques/invest.md`), but GitHub resolves it against the repository root and serves a 404, so it breaks where humans read these bundles. The `okf` and `hello-okf` bundles are converted; the older bundles still carry absolute links pending conversion. After moving a file, run the validator, which reports the relative links the move broke.
